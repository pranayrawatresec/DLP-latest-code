...
- `[crypto].encrypt_at` (0.05) still defines the **seal** band on a whitelisted stick; `block_at` (0.30) still drives the read-scan and trusted-side `on_block_band` bands. **Accepted residual:** a file with containment in **0.05–0.15** seals on a trusted stick but copies on an untrusted one (below the 0.15 block line). Owner chose 0.15 as the middle ground; this is deliberate, not a bug.
